...
const SKILL1_PROMPT = `你是超强阀门的参数提取助手。从客户输入中提取阀门参数。

# 标准阀门参数字段
类型(闸阀/截止阀/止回阀), 口径DN(mm), 压力等级(LB), 数量, 驱动(手轮/伞齿轮/电动//), 连接(RF/BW/SW), 主体材质, 阀瓣阀闸, 阀座, 阀杆轴, 螺柱, 中腔填料, 件号

# 推导规则
- 螺柱: WCB→B7/2H, CF8/CF3→B8/8, CF8M→B8M/8M, LCB→L7/4
- 填料: WCB/CF8/CF8M→304+石墨, LCB/Monel→316+石墨
- 阀杆: 碳钢和件号1#~8#→F6a, 304件→F304, 316件→F316
- 驱动: 止回阀用"/", 闸阀≤150LB且≥16"用伞齿轮
- 标准: 闸阀→API 600, 截止阀→API 600, 止回阀→API 594, 顶装(800/900LB或DN≤50)→API 602
- 闸/顶: DN≤40→顶件(A105), DN>40→闸件(A216 WCB)

# 可换面→件号
H/Cr13对Cr13=1#, Cr13阀瓣+STL阀座=8#, STL对STL=5#, 304/304=2#, Monel=9#, 316/316=10#

# 五环编码（如Z2A3A03-P2G3）
第1项:类型(Z闸J截H止), 第2项:连接(2法兰), 第3项:阀体(A碳钢H304J304L), 第4项:压力(1=150,3=300,5=600), 第5项:件号(A03=8#,A05=5#,B01=2#), 第6项:填料阀杆(G3=石墨杆)

# 材质别名词
碳钢/CS=WCB, 304/18-8=CF8, STL/Stellite/哈斯特/加硬/HF=STL, 13Cr/Cr13/410/F6a/T410=13Cr
...
const SKILL2_PROMPT = `你是超强阀门的BOM生成助手。根据参数查规则表输出14行BOM。

# 规则表1：零部件材质表（按主体材质列查）
| # | 零件 | 碳钢WCB/A105 | 镍铬WC6 | 304 CF8/F304 | 316 CF8M/F316 | 低温LCB | Monel |
|---|------|-------------|---------|-------------|--------------|---------|-------|
| 1 | 阀体 | A216WCB(闸)/A105(顶) | A217WC6 | A351CF8(闸)/F304(顶) | A351CF8M | LCB | Monel |
| 2 | 阀盖 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 |
| 6 | 填料 | 304+柔性石墨 | 304+柔性石墨 | 304+柔性石墨 | 304+柔性石墨 | 316+柔性石墨 | 316+柔性石墨 |
| 7 | 螺柱 | A193B7 | A193B16 | A320B8 | A193B8M | A320-L7 | A193B8M |
| 8 | 螺母 | A194 2H | A194-4 | A194-8 | A194-8M | A194-4 | A194-8M |
| 9 | 填料 | 柔性石墨+缠绕 | 同左 | 同左 | 同左 | 同左 | 同左 |
| 11 | 填料压板 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 |
| 12 | 支承 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 | 同阀体 |
| 13 | 阀杆螺母 | ZQAL9-4 | ZQAL9-4 | ZQAL9-4 | ZQAL9-4 | ZQAL9-4 | ZQAL9-4 |
| 14 | 手轮 | KTH350-10 | 同左 | 同左 | 同左 | 同左 | 同左 |
注：#3阀座、#4阀瓣、#5阀杆、#10填料压套由规则表2决定

# 规则表2：API600件号对应表（按件号查行）
| 件号 | 阀瓣可换面 | 阀座可换面 | 阀杆 | 填料压套 | 名称 |
|--------|----------|----------|------|---------|------|
| 1# | Cr13 | Cr13 | Cr13(=F6a) | Cr13(=12Cr13) | 平装 |
| 5# | HF(STL) | HF(STL) | Cr13(=F6a) | Cr13 | 全堆焊 |
| 8# | Cr13 | HF(STL) | Cr13(=F6a) | Cr13 | 半堆焊 |
| 2# | 304 | 304 | F304 | F304 | 304本体 |
| 9# | Monel | Monel | Monel | Monel | 蒙乃尔 |
| 10# | 316 | 316 | F316 | F316 | 316系 |
| 15# | HF | HF | F304 | F304 | 304堆焊 |
| 16# | HF | HF | F316 | F316 | 316堆焊 |

可换面写法：Cr13→"[基体]+13Cr", HF→"[基体]+STL", 基体取自阀体(碳钢→A105,304→F304)
DN≤40→顶件, DN>40→闸件

# 输入
JSON格式的阀门参数（含主体、件号、DN）

# 输出
严格输出JSON：
{"bom":[{"序号":1,"零件":"阀体","材质":"A105","来源":"牌1"},{"序号":2,"零件":"阀盖","材质":"A105","来源":"牌1"},{"序号":3,"零件":"阀座","材质":"A105+13Cr","来源":"牌2·1#"},...],"牌1":"碳钢A105","牌2":"件号1#(平装)"}`
...
const SEED_PARAMS: Param[] = [
  { id: 'p001', 类型: '闸阀', DN: 100, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+STL', 阀座: 'A105+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '5#', 次数: 11, 数量: 439 },
  { id: 'p002', 类型: '闸阀', DN: 100, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+13Cr', 阀座: 'A105+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '8#', 次数: 17, 数量: 313 },
  { id: 'p003', 类型: '闸阀', DN: 80, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+STL', 阀座: 'A105+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '5#', 次数: 11, 数量: 156 },
  { id: 'p004', 类型: '闸阀', DN: 50, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+13Cr', 阀座: 'A105+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '8#', 次数: 11, 数量: 151 },
  { id: 'p005', 类型: '闸阀', DN: 50, 压力: 300, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+13Cr', 阀座: 'A105+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '8#', 次数: 10, 数量: 72 },
  { id: 'p006', 类型: '闸阀', DN: 50, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'CF8', 阀瓣阀闸: 'CF8+STL', 阀座: '本体+STL', 阀杆轴: 'F304', 螺柱: 'B8/8', 中腔填料: '316+石墨', 设计标准: 'API 600', 件号: '15#', 次数: 4, 数量: 18 },
  { id: 'p007', 类型: '截止阀', DN: 50, 压力: 150, 驱动: '手轮', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'A105+13Cr', 阀座: '本体+STL', 阀杆轴: 'F6a', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 600', 件号: '8#', 次数: 6, 数量: 21 },
  { id: 'p008', 类型: '止回阀', DN: 80, 压力: 150, 驱动: '/', 连接: 'RF', 主体: 'WCB', 阀瓣阀闸: 'WCB+13Cr', 阀座: '本体+STL', 阀杆轴: '/', 螺柱: 'B7/2H', 中腔填料: '304+石墨', 设计标准: 'API 594', 件号: '8#', 次数: 5, 数量: 30 },
]
...
const TRIM_TABLE: TrimRow[] = [
  { 号: '1#', 阀瓣: 'Cr13', 阀座: 'Cr13', 阀杆: 'Cr13(F6a)', 压套: 'Cr13(12Cr13)', 名称: '平装' },
  { 号: '2#', 阀瓣: '304', 阀座: '304', 阀杆: 'F304', 压套: 'F304', 名称: '304本体' },
  { 号: '5#', 阀瓣: 'HF(STL)', 阀座: 'HF(STL)', 阀杆: 'Cr13(F6a)', 压套: 'Cr13', 名称: '全堆焊' },
  { 号: '8#', 阀瓣: 'Cr13', 阀座: 'HF(STL)', 阀杆: 'Cr13(F6a)', 压套: 'Cr13', 名称: '半堆焊' },
  { 号: '9#', 阀瓣: 'Monel', 阀座: 'Monel', 阀杆: 'Monel', 压套: 'Monel', 名称: '蒙乃尔' },
  { 号: '10#', 阀瓣: '316', 阀座: '316', 阀杆: 'F316', 压套: 'F316', 名称: '316系' },
  { 号: '12#', 阀瓣: '316', 阀座: 'HF(STL)', 阀杆: 'F316', 压套: 'F316', 名称: '316半堆焊' },
  { 号: '15#', 阀瓣: 'HF', 阀座: 'HF', 阀杆: 'F304', 压套: 'F304', 名称: '304堆焊' },
  { 号: '16#', 阀瓣: 'HF', 阀座: 'HF', 阀杆: 'F316', 压套: 'F316', 名称: '316堆焊' },
]
...
function PageNewQuote({ params, setQuotes, setPage }: {
  ...
  const handleExtract = async () => {
    ...
    try {
      const raw = await callAI(SKILL1_PROMPT, input)
  ...
  const handleGenBOM = async () => {
    ...
    try {
      ...
      for (const item of extracted) {
        const raw = await callAI(SKILL2_PROMPT, JSON.stringify(item))
        const parsed = parseJSON(raw)
