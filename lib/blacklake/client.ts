...
export async function blFetch<T = unknown>(path: string, body: unknown): Promise<BlResponse<T>> {
  ...
  let resp = await call<T>(path, body, cachedToken!)
  // token 失效：code=401 / subCode=USER_VERIFICATION_ERROR → 重登一次
  if (resp.code === '401' || resp.subCode === 'USER_VERIFICATION_ERROR') {
    await login()
    resp = await call<T>(path, body, cachedToken!)
  }
  if (resp.code !== OK) {
...
/** 供测试/排障：清掉缓存 token。 */
export function resetToken() { cachedToken = null }
