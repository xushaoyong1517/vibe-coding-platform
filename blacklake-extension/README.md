...
| `kitting.*` | 齐套分析：按订单齐套率/缺料 + 缺料采购汇总（基于 订单/BOM/库存 计算） |
...
2. 弹窗点 **全量同步**（首次）/ **增量同步**。
3. 弹窗 → **📊 齐套分析**：按订单看齐套率/缺料、缺料采购汇总；或 **🔎 数据浏览** 切表/搜索/只读 SQL。

## 齐套分析逻辑
- 需求 = 销售订单行的「待排产数量」(pending_qty，回落订单数量)，仅取 >0 且有父件产品的行。
- 单层 BOM 炸开（`materials` 父→子 + 单位用量），按**交期升序统筹扣减**产品库存（`products.stock_qty`）。
- 每行判定 **齐套 / 缺料 / 无BOM**，给出齐套率与缺料项；「缺料采购」汇总各子件缺口合计、现有库存、受影响订单数。
- 移植自主项目 `lib/kitting.ts`，纯前端计算（页内 runSql 取数）。
